...
def kz_get_info(update: Update, context: CallbackContext):
  ...
  if update.message.text.lower() in ["Құжаттарды қабылдау".lower()]:
    ...
    update.message.reply_text("Конкурстық іріктеу нәтижелерімен мына сілтеме бойынша танысуға болады: https://www.nis.edu.kz/kz/applicants/konkurs-results/", reply_markup=kz_custom_keyboard)
    return STATE1
  elif update.message.text.lower() in ["Байланыстар мен мекенжай".lower()]:
    update.message.reply_text("Мекен-жайы: Петропавл қ., көш. Ибраева, 22А\nТелефон: +7 (7152) 55-97-22", reply_markup=kz_custom_keyboard)
    return STATE1
  elif update.message.text.lower() in ["Бот туралы".lower()]:
    update.message.reply_text("Бот Петропавл қаласындағы НЗМ ХБН пайдалану үшін әзірленген.\nӘзірлеушілер:\nАсхат Аубакиров, aubakirov_a@ptr.nis.edu.kz,\nhttps://www.linkedin.com/in/askhattio/\nАзат Кабиден", reply_markup=kz_custom_keyboard)
    return STATE1
  elif update.message.text.lower() in ["Сұрақ қою".lower()]:
    update.message.reply_text("Барлық сұрақтар бойынша +7(7152) 55-97-22 телефонына хабарласыңыз немесе aubakirov_a@ptr.nis.edu.kz электронды поштасына жазыңыз.", reply_markup=kz_custom_keyboard)
    return STATE1
  elif update.message.text.lower() in ["Қосымша: Солтүстік Қазақстан облысына арналған интернат".lower()]:
    update.message.reply_text("Интернат Солтүстік Қазақстан облысынан келген үміткерлерге беріледі", reply_markup=kz_custom_keyboard)
    return STATE1
  elif update.message.text.lower() in ["Жиі қойылатын сұрақтар".lower()]:
    update.message.reply_text("1) Қандай жағдайларда құжаттарды қабылдаудан бас тартуға болады?\nҚұжаттарды қабылдаудан бас тартуға мыналар негіз болуы мүмкін:\n- конкурстық іріктеуге қатысуға өтінімді белгіленген мерзімнен кешіктіру;\n- толық емес құжат ұсыну. құжаттар тізімі.\n\n2) Құжаттарды басқа тұлға арқылы беруге бола ма?\n-Қабылдау комиссиясына үшінші тұлғаның сенімхатсыз берген құжаттары қабылданбайды. Егер адамның қол қоюға құқығы бар сенімхаты болса, онда құжаттар қабылданады.", reply_markup=kz_custom_keyboard)
    return STATE1
  elif update.message.text.lower() in ["Шығу".lower()]:
    update.message.reply_text("Бот аяқталуда.\nҚайтадан бастау үшін /start жіберіңіз", reply_markup=ReplyKeyboardRemove())
    return ConversationHandler.END


def cancel(update: Update, context: CallbackContext):
  update.message.reply_text(f"Canceled. To start over, type /start",
                            reply_markup=ReplyKeyboardRemove())
  return ConversationHandler.END


choice_regex = re.compile(r"^(Қазақ тілі|Русский)$", re.IGNORECASE)
